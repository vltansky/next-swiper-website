...
<div className="important-note">
  ...
  elements (e.g. without `navigation.nextEl`, `pagination.el`, etc.)
</div>
...
```html
<Swiper onSlideChange={() => {/*...*/}} onReachEnd={() => {/*...*/}} ... >
```
...
```html
import SwiperCore, { EffectFade } from "swiper"; SwiperCore.use([EffectFade]);
```
...
import SwiperCore, { EffectFade } from "swiper"; import "swiper/swiper.scss";
import "swiper/components/effect-fade/effect-fade.scss";
SwiperCore.use([EffectFade]); export default () => { return (
<Swiper effect="fade">
  {[1, 2, 3].map((i, el) => { return <SwiperSlide>Slide {el}</SwiperSlide>; })}
</Swiper>
); };
```
